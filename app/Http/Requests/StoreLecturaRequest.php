...
class StoreLecturaRequest extends FormRequest
{
    ...
    {
        return [
            'id_operador' => 'required|integer', // Validar que exista en la tabla `operadores`
            'id_toma' => 'required|integer', // Validar que exista en la tabla `tomas`
            'id_periodo' => 'required', // Validar que exista en la tabla `periodos`
            'id_origen' => 'required|integer', // Cambiar a `exists:tabla_origen,id` si aplica
            'modelo_origen' => 'required|string', // Ajustar longitud máxima según corresponda
            'id_anomalia' => 'nullable|integer', // Puede ser opcional o requerido según lógica
            'lectura' => 'required|numeric|min:0', // Validar que sea numérico y mínimo 0
